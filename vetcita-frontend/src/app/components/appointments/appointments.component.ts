    ...
    <div class="max-w-4xl mx-auto relative">
      ...
      <div class="mb-8 bg-white rounded-3xl p-6 shadow-sm border border-sky-100 flex items-center justify-between">
        <img src="/assets/images/logo.png"
             ...
             onerror="this.src='https://via.placeholder.com/150x50?text=VET+CITA'">
      </div>
      ...
      <div class="flex gap-4 mb-6">
        <button
          ...
          [class.bg-sky-600]="activeTab() === 'upcoming'"
          [class.text-white]="activeTab() === 'upcoming'"
          [class.bg-slate-200]="activeTab() !== 'upcoming'"
          [class.text-slate-700]="activeTab() !== 'upcoming'"
          class="px-6 py-2 rounded-full font-semibold transition-colors">
          Próximas citas
        </button>
        <button
          (click)="setActiveTab('history')"
          [class.bg-sky-600]="activeTab() === 'history'"
          [class.text-white]="activeTab() === 'history'"
          [class.bg-slate-200]="activeTab() !== 'history'"
            ...
            <div class="bg-white rounded-2xl p-5 flex flex-col sm:flex-row sm:items-center justify-between shadow-sm border border-slate-200 hover:border-sky-200 transition-colors gap-4">
              <div>
                ...
                <p class="text-sm text-slate-700">
                  Paciente: <span class="font-semibold">{{ appt.petName }} ({{ appt.petSpecies }})</span>
                </p>
                <p class="text-sm text-slate-700">Servicio: {{ appt.serviceName }}</p>
              </div>
              <div class="flex gap-3 self-end sm:self-auto">
                <!-- VER -->
                <button (click)="openModal(appt)" title="Ver Detalles" class="w-10 h-10 rounded-full bg-sky-100 text-sky-600 flex items-center justify-center hover:bg-sky-200 transition">
                ...
                @if (activeTab() === 'upcoming' && appt.status !== 'CANCELLED') {
                  <button (click)="openEditModal(appt)" title="Editar Cita" class="w-10 h-10 rounded-full bg-amber-100 text-amber-600 flex items-center justify-center hover:bg-amber-200 transition">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
                ...
                <!-- CANCELAR -->
                @if (activeTab() === 'upcoming' && appt.status !== 'CANCELLED') {
                  <button (click)="cancelAppointment(appt.id)" title="Cancelar Cita" class="w-10 h-10 rounded-full bg-rose-100 text-rose-600 flex items-center justify-center hover:bg-rose-200 transition">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
      ...
      @if (selectedAppointment()) {
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
          <div class="bg-white rounded-3xl p-6 md:p-8 max-w-md w-full shadow-2xl relative">
            ...
            <div class="mt-8">
              <button (click)="closeModal()" class="w-full py-3 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl font-bold transition">Cerrar</button>
            </div>
          </div>
        </div>
      }

      <!-- MODAL EDITAR (con grid de horas como en agendamiento) -->
      @if (editingAppointment()) {
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
          <div class="bg-white rounded-3xl p-6 w-full max-w-md shadow-2xl">
            <h2 class="text-2xl font-bold text-slate-800 mb-6">Editar cita</h2>
            <div class="space-y-4">
              <!-- Fecha -->
              <div>
                <label class="block text-sm font-semibold mb-2">Nueva fecha</label>
                <input
                  type="date"
                  [min]="minDateForEdit"
                  [(ngModel)]="newDate"
                  (change)="onDateChange(newDate)"
                  class="w-full border border-slate-300 rounded-xl px-4 py-3">
              </div>

              <!-- Horas disponibles (grid de botones) -->
              <div>
                <label class="block text-sm font-semibold mb-2">Nueva hora</label>
                @if (isLoadingTimes) {
                  <div class="text-sm text-slate-500 text-center py-4">Calculando disponibilidad...</div>
                } @else if (availableTimes.length === 0 && newDate) {
                  <div class="text-sm text-rose-500 text-center py-4">No hay horas disponibles para esta fecha.</div>
                } @else if (!newDate) {
                  <div class="text-sm text-slate-500 text-center py-4">Selecciona una fecha para ver las horas.</div>
                } @else {
                  <div class="grid grid-cols-3 gap-3">
                    @for (time of availableTimes; track time) {
                      <button
                        ...
                        [class.bg-[#0e7490]]="newTime === time"
                        [class.text-white]="newTime === time"
                        [class.border-[#0e7490]]="newTime === time"
                        [class.bg-slate-100]="newTime !== time"
                        [class.text-slate-700]="newTime !== time"
                        [class.hover:bg-slate-200]="newTime !== time">
                        {{ time }}
                      </button>
                    }
                  </div>
            ...
            <div class="flex justify-end gap-3 mt-8">
              <button (click)="closeEditModal()" class="px-5 py-3 rounded-xl bg-slate-200 hover:bg-slate-300 font-semibold">Cancelar</button>
              <button (click)="saveAppointmentChanges()" [disabled]="!newDate || !newTime" class="px-5 py-3 rounded-xl bg-sky-600 hover:bg-sky-700 disabled:bg-slate-400 text-white font-semibold">Guardar cambios</button>
            </div>
          </div>
        </div>
      }
...
export class AppointmentsComponent implements OnInit {
  ...
  minDateForEdit: string = new Date().toISOString().split('T')[0];
  ...
  openEditModal(appointment: any) {
    ...
    // Cargar horas disponibles para la fecha actual de la cita
    this.loadAvailableTimes(appointment.vetId, appointment.date);
    document.body.style.overflow = 'hidden';
  }
  ...
  loadAvailableTimes(vetId: number, date: string) {
