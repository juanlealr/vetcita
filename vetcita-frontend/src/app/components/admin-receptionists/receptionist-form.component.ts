...
export class ReceptionistFormComponent implements OnInit {
  ...
  onSubmit() {
    ...
    if (this.isEditMode && this.repId) {
      const updateData = {
        firstName: this.repForm.get('firstName')?.value,
        lastName: this.repForm.get('lastName')?.value,
        email: this.repForm.get('email')?.value,
        phone: this.repForm.get('phone')?.value,
        isActive: this.repForm.get('active')?.value
      };
      
      this.receptionistService.updateReceptionist(this.repId, updateData).subscribe({
        next: () => {
      ...
      const createData = {
        firstName: this.repForm.get('firstName')?.value,
        lastName: this.repForm.get('lastName')?.value,
        identificationType: this.repForm.get('identificationType')?.value,
        identificationNumber: this.repForm.get('identificationNumber')?.value,
        email: this.repForm.get('email')?.value,
        phone: this.repForm.get('phone')?.value,
        password: this.repForm.get('password')?.value
      };
      
      this.receptionistService.createReceptionist(createData).subscribe({
        next: () => {
        ...
        error: (error) => {
          this.isSubmitting = false;
          const errorMessage = error?.error?.message || 'Hubo un problema al registrar.';
