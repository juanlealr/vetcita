              ...
              @if (filteredVets.length === 0) {
                <tr>
                  <td colspan="5" class="px-6 py-8 text-center text-slate-500">
                    No se encontraron veterinarios en la base de datos.
                  </td>
                </tr>
              }
              
              <tr *ngFor="let vet of filteredVets" class="hover:bg-slate-50 transition" [class.opacity-60]="!vet.active">
                <td class="px-6 py-4">
                ...
                <td class="px-6 py-4">
                  <div class="flex items-center justify-center gap-3">
                    ...
                    <button (click)="toggleStatus(vet)" [class]="vet.active ? 'text-rose-400 hover:text-rose-600' : 'text-emerald-400 hover:text-emerald-600'" class="transition cursor-pointer" [title]="vet.active ? 'Desactivar' : 'Activar'">
                      <svg *ngIf="vet.active" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-5 h-5">
                        <path fill-rule="evenodd" d="M12 2.25c-5.385 0-9.75 4.365-9.75 9.75s4.365 9.75 9.75 9.75 9.75-4.365 9.75-9.75S17.385 2.25 12 2.25Zm-1.72 6.97a.75.75 0 1 0-1.06 1.06L10.94 12l-1.72 1.72a.75.75 0 1 0 1.06 1.06L12 13.06l1.72 1.72a.75.75 0 1 0 1.06-1.06L13.06 12l1.72-1.72a.75.75 0 1 0-1.06-1.06L12 10.94l-1.72-1.72Z" clip-rule="evenodd" />
                      </svg>
                      <svg *ngIf="!vet.active" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-5 h-5">
                        <path fill-rule="evenodd" d="M2.25 12c0-5.385 4.365-9.75 9.75-9.75s9.75 4.365 9.75 9.75-4.365 9.75-9.75 9.75S2.25 17.385 2.25 12Zm13.36-1.814a.75.75 0 1 0-1.22-.872l-3.236 4.53L9.53 11.22a.75.75 0 0 0-1.06 1.06l2.25 2.25a.75.75 0 0 0 1.14-.094l3.75-5.25Z" clip-rule="evenodd" />
                      </svg>
                    </button>
                  </div>
                </td>
      ...
      if (result.isConfirmed) {
        this.vetService.toggleVetStatus(vet.id!).subscribe({
          next: () => {
            ...
            Swal.fire(
            '¡Actualizado!',
            `El estado del veterinario ha sido modificado.`,
            'success'
            );
        }
        });
